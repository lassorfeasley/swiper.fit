...
export default function JoinTrainerInvitation({ inviterName = 'John Smith', email = '' }) {
  ...
  const signupUrl = `https://www.swiper.fit/create-account?email=${encodeURIComponent(email)}&fromInvite=trainer`;

  return React.createElement(
    DefaultLayout,
    {
      title: '',
      preheader: `${inviterName} has invited you to be their trainer on Swiper`,
      hideTitle: true,
    },
    // Logo checkmark
    ...
    React.createElement('div', { style: { ...bodyStyle, marginTop: 4 } },
      `${inviterName} has invited you to be their trainer on Swiper. As their trainer, you'll be able to help them with their fitness journey.`
    ),
