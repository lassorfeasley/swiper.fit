...
export default function ActionPill({ 
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`ActionPill h-10 ${showText ? 'min-w-24' : 'min-w-10'} ${showText ? 'px-4 py-3' : 'py-3'} ${backgroundColor} ${showText ? 'rounded-full' : 'rounded-[20px]'} inline-flex justify-center items-center gap-0.5 whitespace-nowrap ${className}`}
      aria-label={label}
    >
