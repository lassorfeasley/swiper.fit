...
const ConfigureProgramExercisesIndex = () => {
    ...
    try {
      ...
      if (!exercise_id) {
        const { data: newEx, error: insertError } = await supabase
          ...
          .select('id')
          .single();
        if (insertError || !newEx) throw new Error('Failed to create exercise: ' + (insertError?.message || 'Unknown error'));
      ...
      const { data: progEx, error: progExError } = await supabase
        ...
        .select('id')
        .single();
      if (progExError || !progEx) throw new Error('Failed to link exercise to program: ' + (progExError?.message || 'Unknown error'));
  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      />
      <CardWrapper className="px-4" reorderable={true} items={filteredExercises} onReorder={setExercises}>
        {loading ? (
